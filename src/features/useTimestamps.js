...
export const useTimestamps = () => {
  ...
  const state = reactive({
    timeFormatter: Intl.DateTimeFormat("fr", {
      hour: "2-digit",
      minute: "2-digit",
    }),
    fullDateFormatter: Intl.DateTimeFormat("fr", { dateStyle: "full" }),
  });
  //Permet de formatter un timestamps en simple données de temps
  const formatTimestamp = function (ts) {
    return state.timeFormatter.format(ts);
  };
  //Permet de récupérer la durée formattée entre 2 timestamps
